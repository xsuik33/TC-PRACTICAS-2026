...
def chip(label: str, color: str = ACCENT) -> ft.Container:
    return ft.Container(
        content=ft.Text(label, size=11, color=color, font_family=FONT),
        bgcolor=f"{color}22",
...
def section_title(text: str) -> ft.Text:
    return ft.Text(text, size=13, color=ACCENT, font_family=FONT, weight="bold")
...
def main(page: ft.Page):
    ...
    def build_transition_table(_=None):
        ...
        # Aquí eliminé los 'alignment' problemáticos que causaban el error rojo
        ...
        for state in states:
            cells = [ft.Container(
                ft.Text(state, color=TEXT, font_family=FONT, size=12, weight="bold"),
                width=80
            )]
            for sym in alphabet:
                tf = ft.TextField(
    ...
    def cargar_archivo_manual(e):
        ...
        try:
            ...
            import_result.controls.append(
                ft.Text(f"✓ Autómata '{afd.name}' cargado correctamente", color=ACCENT2, font_family=FONT, size=13, weight="bold")
            )
            import_result.controls.append(
        ...
        sim_result_container.controls.append(
            ft.Container(
                ft.Row([ft.Text(f'"{string}"  →  {symbol}',
                               color=color, font_family=FONT, size=16, weight="bold")],
                       alignment="center"),
        ...
        sim_result_container.controls.append(
            ft.Container(
                ft.Row([ft.Text(f'"{string}"  →  Paso {last["step"]}  |  {status_text}',
                               color=color, font_family=FONT, size=14, weight="bold")],
                       alignment="center"),
    ...
    sim_tab = ft.Column([
        ...
        ft.Row([
            ...
            ft.ElevatedButton("⏭  Paso a paso", on_click=step_forward,
                              bgcolor=ACCENT, color=BG),
            ft.ElevatedButton("⏮  Reiniciar", on_click=lambda _: (
                trace_data.clear(), sim_result_container.controls.clear(),
                setattr(current_step, '__setitem__', None),
                page.update()
            ),
    ...
    def run_str_ops(_=None):
        ...
        def make_list(title, items, color):
            ...
            return ft.Column([
                ft.Text(f"{title} ({len(items)})", color=color, font_family=FONT, size=12, weight="bold"),
                chips_row,
            ], spacing=4)
    ...
    def run_kleene(kind: str):
        ...
        kleene_result.controls.append(
            ft.Text(title_str, color=ACCENT, font_family=FONT, size=12, weight="bold")
        )
        chips = ft.Row(
            [ft.Container(
    ...
    def refresh_afd_info():
        ...
        if not afd.initial_state:
            ...
            afd_info_panel.controls.extend([
                ft.Text(afd.name, color=TEXT, font_family=FONT, size=13, weight="bold"),
    ...
    def switch_tab(idx: int):
        ...
        for i, btn in enumerate(tab_buttons):
            ...
            btn.color = BG if i == idx else TEXT2
    ...
    header = ft.Container(
        ft.Row([
            ft.Text("AFD", color=ACCENT, font_family=FONT, size=22, weight="bold"),
    ...
    sidebar = ft.Container(
        ft.Column([
            ft.Text("AFD ACTIVO", color=TEXT2, font_family=FONT, size=10, weight="bold"),
            divider(),
            afd_info_panel,
        ], spacing=8),
